...
@section('title','Edit Review Item')
@section('body')
                        ...
                        <div class="checkout_details_area mt-50 clearfix">
                            ...
                            <div class="cart-title">
                                <h2>Edit {{$item->name}}'s Review</h2>
                                        ...
                                        @endif
                            <form method="POST" action="{{ url('item/review/update')}}" enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                <div class="row">
                                    ...
                                    <div class="col-12 mb-3 {{ ($errors->has('comment')) ? 'has-error' : ''}}">
                                        ...
                                        <textarea name="comment"  class="form-control" placeholder="Comment on Item" id="" cols="30" rows="10" maxlength="250">{{old('comment', $review->comment)}}</textarea>
                                    ...
                                    <div class="col-12 mb-3 {{ ($errors->has('rating')) ? 'has-error' : ''}}" style="padding-bottom: 20px;">
                                        ...
                                        @foreach($ratings as $rating)
                                                <option value="{{$rating}}" {{ (old("rating", $review->rating) == $rating ? "selected" : " ") }}>{{$rating}}</option>
                                        @endforeach
                                        </select>
